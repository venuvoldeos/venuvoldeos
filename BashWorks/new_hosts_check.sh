...
hosts_file="/users/builders/venugona_scripts/new-fxos_hosts_list.txt"
hosts=()
...
check_disk_usage() {
  host="$1"
  for partition in "${partitions[@]}"; do
    output=$(ssh "$host" "df -kh $partition" 2>/dev/null)
    if [[ $? -ne 0 ]]; then
      echo "**************** ALERT !!! ****************" | tee -a "$alert_file"
      echo "ALERT !!! Login to $host failed" | tee -a "$alert_file"
      echo "********************************" | tee -a "$alert_file"
    ...
    # Check if the output contains the header line, and skip it
    if [[ "$output" == *Filesystem*Size*Used*Avail*Use%* ]]; then
